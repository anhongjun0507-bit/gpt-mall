...
NEXT_PUBLIC_SITE_URL=https://www.digitalst.kr      # redirectTo / OAuth redirect_uri 베이스 (개발: http://localhost:3000)
                                                   # apex(digitalst.kr)는 www 로 307 redirect 되므로 반드시 www 로 둘 것
```
...
#### 카카오 Client Secret 발급 방법 (클라이언트가 직접)
1. https://developers.kakao.com → 내 애플리케이션 → **보안** 메뉴
2. **Client Secret 코드 생성** 클릭
3. 발급된 코드 **활성화 ON**
4. 생성된 코드를 카톡으로 프리즘에 전달

※ Client Secret 없이도 카카오 로그인 작동은 가능하지만 보안 강화를 위해 발급 권장.
